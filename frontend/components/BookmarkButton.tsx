...
interface TopBarProps {
  backLabel?: string;
  onBackAction?: () => void;  // 自定义返回行为（如"重试"）
    ...
    <div className="flex items-center justify-between mb-6">
      ...
      {onBackAction ? (
        <button
          ...
          className="group inline-flex items-center gap-1.5 rounded-xl border border-purple-200 bg-purple-50 px-4 py-2 text-sm font-medium text-purple-600 shadow-sm transition-all hover:border-purple-400 hover:bg-purple-100 hover:shadow-md active:scale-[0.98]"
        >
          <svg className="h-4 w-4 transition-transform group-hover:-rotate-180 duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
